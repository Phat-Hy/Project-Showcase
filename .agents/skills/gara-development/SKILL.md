...
- **Backend**: C# ASP.NET Core Web API with JWT Token authentication.
- **Database**: PostgreSQL (specifically Azure Database for PostgreSQL Flexible Server as IaC).
- **Authentication**: JWT Cookie-based auth with Password hashing.
...
- `status`: Enum ('Active', 'Draft', 'At-Risk', 'Suspended')
...
- **Infrastructure as Code**: Provision resources using Azure Terraform.
- **CI/CD Pipeline**: Deploy via GitHub Actions using federated credentials (OIDC).
